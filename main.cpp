...
int main()
{
  ...
  for (lambda = 1; lambda <= 10; lambda++){
	  ...
	  Elist.insert(exp_rv(lambda),ARR, QUEUE1, TBD); // Generate first arrival event to queue 1 for high priority customer
	  ...
	  while (!done)
	  {
	  	CurrentEvent = Elist.get();               // Get next Event from list
			...
			Elist.insert(clock+exp_rv(lambda),ARR, QUEUE1, TBD);
